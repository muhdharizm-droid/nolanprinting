    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-gradient-to-r from-slate-900 via-blue-950 to-indigo-950 p-6 rounded-3xl text-white shadow-xl shadow-slate-900/10 dark:from-slate-950 dark:via-blue-950/80 dark:to-indigo-950/70 border border-white/5">
        <div>
          ...
          <p className="text-slate-300 text-xs mt-1">{t("realtime_overview_subtitle")}</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3.5">
          ...
          <>
            ...
            <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col justify-between transition-colors">
              ...
              <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold mt-1">
                {summary?.totalTransactions} {t("transactions_count")}
              </span>
            </div>
        ...
        <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col justify-between transition-colors">
          ...
          <span className="text-[10px] text-slate-400 dark:text-slate-500">{t("direct_material_cost")}</span>
        </div>
        ...
        <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col justify-between transition-colors">
          ...
          <span className="text-[10px] text-slate-400 dark:text-slate-500">{t("operational_costs")}</span>
        </div>
        ...
        <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col justify-between transition-colors">
          ...
          <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold">
            {summary?.profitMargin}% {t("profit_margin")}
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            {chartLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip
                    formatter={(val: any, name: any) => [formatMYR(val), name === "revenue" ? t("revenue") : t("gross_profit")]}
                    contentStyle={{
        ...
        <div className="lg:col-span-4 bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col transition-colors">
          <div className="mb-3">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500">{t("revenue_split_desc")}</p>
          </div>
          <div className="flex-1 flex flex-col items-center justify-center">
            {categoryData.length === 0 ? (
              <div className="text-slate-400 dark:text-slate-500 text-xs font-medium py-10">{t("no_categories_found")}</div>
            ) : (
              <>
                <div className="h-44 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      <Tooltip
                        formatter={(val: any, name: any) => [formatMYR(val), translateCategory(name, language)]}
                        contentStyle={{
                          backgroundColor: "#0f172a",
                          color: "#ffffff",
                          borderRadius: "12px",
                  ...
                  {categoryData.map((cat, idx) => (
                    <div key={cat.name} className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="text-slate-600 dark:text-slate-400 truncate max-w-[120px]">{translateCategory(cat.name, language)}</span>
                      </div>
